CREATE OR REPLACE FUNCTION mishipay_item_inventory_updateRecords1(in a_no_of_records integer,v_item_name varchar(255)) RETURNS integer AS $$
DECLARE
	v_item_id integer;
    v_counter integer;
	  	...
	  	loop
			...
			for counter in 1..2 loop
				...
				RAISE NOTICE 'v_item_id : % :v_productname :% ',v_item_id, v_productname;
					--INSERT INTO mishipay_item_inventory(item_name,store_id,category_id,country_id,retailor_id,score) 
					--VALUES (v_productname,v_store_id ,v_category_id,(SELECT floor(random()*(5-1+1)+1)),v_retailer_id,		
					--	(SELECT floor(random()*(5-1+1)+1)));
				UPDATE mishipay_item_inventory SET item_name = v_productname
...
select * from mishipay_item_inventory_updateRecords1(2,'Speaker');
select * from mishipay_item_inventory where item_name ='Speaker3';
---Check Data
...
CREATE OR REPLACE FUNCTION mishipay_item_inventory_index_updateRecords1(in a_no_of_records integer,v_item_name varchar(255)) RETURNS integer AS $$
DECLARE
	v_item_id integer;
    v_counter integer;
    v_productname varchar(255);
	  	...
	  	loop
			...
			for counter in 1..2 loop
				...
				RAISE NOTICE 'v_item_id : % :v_productname :% ',v_item_id, v_productname;
					...
					--VALUES (v_productname,v_store_id ,v_category_id,(SELECT floor(random()*(5-1+1)+1)),v_retailer_id,		
					--	(SELECT floor(random()*(5-1+1)+1)));
				UPDATE mishipay_item_inventory_index SET item_name = v_productname
				WHERE   store_id='e3ded2c4-5aca-46c2-b178-fb4ce7a11a46' 
				and item_id =v_item_id;
				ELSE
					exit;
...
$$ LANGUAGE plpgsql;
--8) Run to create data 
select * from mishipay_item_inventory_index_InsertRecords1(20);
...
CREATE OR REPLACE FUNCTION mishipay_item_inventory_store_dpt_index_updateRecords1(in a_no_of_records integer,v_item_name varchar(255)) RETURNS integer AS $$
DECLARE
	v_item_id integer;
    v_counter integer;
    v_productname varchar(255);
	  	...
	  	loop
			...
			for counter in 1..2 loop
				...
				RAISE NOTICE 'v_item_id : % :v_productname :% ',v_item_id, v_productname;
					...
					--VALUES (v_productname,v_store_id ,v_category_id,(SELECT floor(random()*(5-1+1)+1)),v_retailer_id,		
					--	(SELECT floor(random()*(5-1+1)+1)));
				UPDATE mishipay_item_inventory_store_dpt SET item_name = v_productname
				WHERE   store_id='e3ded2c4-5aca-46c2-b178-fb4ce7a11a46' 
				and item_id =v_item_id;
				ELSE
					exit;
...
--8) Run to create data 
select * from mishipay_item_inventory_store_dpt_index_updateRecords1(10,'Chocolate');

---Check Data
select * from mishipay_item_inventory_store_dpt where item_id=25051
item_name Like 'Chotolate%'	

select * from mishipay_item_inventory_store_dpt where 
