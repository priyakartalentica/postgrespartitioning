...
create sequence mishipay_item_inventory_item_id_seq;
--CREATE SEQUENCE table_name_id_seq;
--alter table mishipay_item_inventory alter id set default nextval('mishipay_items_iteminventory_test_id_seq');
--Select setval('mishipay_items_iteminventory_test_id_seq', 1168584968 ); --set to the highest current value of playerID





CREATE TABLE public.mishipay_item_inventory
(
    item_id integer NOT NULL DEFAULT nextval('mishipay_item_inventory_item_id_seq'),
    ...
    retailor_id uuid,
    score integer,
    CONSTRAINT mishipay_item_inventory_pkey PRIMARY KEY (item_id),
    CONSTRAINT fk_item_mishipay_inventory_category_id FOREIGN KEY (category_id)
        REFERENCES public.category (category_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_item_mishipay_inventory_country_id FOREIGN KEY (country_id)
        REFERENCES public.country (country_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_item_mishipay_inventory_retailer_id FOREIGN KEY (retailor_id)
        REFERENCES public.retailor (retailor_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_item_mishipay_inventory_store_id FOREIGN KEY (store_id)
...
CREATE OR REPLACE FUNCTION mishipay_item_inventory_InsertRecords(in a_no_of_records integer) RETURNS integer AS $$
DECLARE
    v_counter integer;
...
select * from mishipay_item_inventory_InsertRecords(1);
