...
create sequence mishipay_item_inventory_index_item_id_seq;


CREATE TABLE public.mishipay_item_inventory_index
(
    item_id integer NOT NULL DEFAULT nextval('mishipay_item_inventory_index_item_id_seq'),
    ...
    retailor_id uuid,
    score integer,
    CONSTRAINT mishipay_item_inventory_index_pkey PRIMARY KEY (item_id),
    CONSTRAINT fk_category_mishipay_item_inventory_index FOREIGN KEY (category_id)
        REFERENCES public.category (category_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_country_mishipay_item_inventory_index FOREIGN KEY (country_id)
        REFERENCES public.country (country_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_retailor_mishipay_item_inventory_index FOREIGN KEY (retailor_id)
        REFERENCES public.retailor (retailor_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_store_item_mishipay_inventory_index FOREIGN KEY (store_id)
...
ALTER TABLE public.mishipay_item_inventory_index
    OWNER to postgres;
-- Index: item_inventory_index_category_id

-- DROP INDEX public.item_inventory_index_category_id;

CREATE INDEX mishipay_item_inventory_index_category_id
    ON public.mishipay_item_inventory_index USING btree
...
CREATE INDEX mishipay_item_inventory_index_country_id
    ON public.mishipay_item_inventory_index USING btree
...
CREATE INDEX mishipay_item_inventory_index_item_name
    ON public.mishipay_item_inventory_index USING btree
...
CREATE INDEX mishipay_item_inventory_index_retailor_id
    ON public.mishipay_item_inventory_index USING btree
...
CREATE INDEX mishipay_item_inventory_index_store_id
    ON public.mishipay_item_inventory_index USING btree
...
CREATE OR REPLACE FUNCTION mishipay_item_inventory_index_InsertRecords(in a_no_of_records integer) RETURNS integer AS $$
DECLARE
    v_counter integer;
	    ...
	    open c_retailor;
	  	loop
			...
			for counter in 1..998 loop
				...
				INSERT INTO mishipay_item_inventory_index(item_name,store_id,category_id,country_id,retailor_id,score) 
...
select * from mishipay_item_inventory_index_InsertRecords(1);
select count(*) from mishipay_item_inventory_index;
select * from mishipay_item_inventory_index limit 20;
